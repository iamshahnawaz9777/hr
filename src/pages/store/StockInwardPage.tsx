...
export default function StockInwardPage() {
  ...
  const [form, setForm] = useState({ item_id: '', quantity: '', supplier_vendor: '', reference_no: '', transaction_date: format(new Date(), 'yyyy-MM-dd'), notes: '' });
  ...
  const fetchAll = async () => {
    ...
    const [txRes, itemRes] = await Promise.all([
      supabase.from('stock_transactions').select('*, item:items!stock_transactions_item_id_fkey(id, item_code, name, unit)').eq('transaction_type', 'inward').order('created_at', { ascending: false }).limit(100),
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('stock_transactions').insert({
      item_id: form.item_id, transaction_type: 'inward', quantity: qty,
      supplier_vendor: form.supplier_vendor || null, reference_no: form.reference_no || null,
    ...
    setShowDialog(false);
    setForm({ item_id: '', quantity: '', supplier_vendor: '', reference_no: '', transaction_date: format(new Date(), 'yyyy-MM-dd'), notes: '' });
    fetchAll();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-bold flex items-center gap-2"><ArrowDownToLine className="w-5 h-5 text-success" /> Stock Inward</h1>
        {canWrite && <Button size="sm" onClick={() => setShowDialog(true)}><Plus className="w-4 h-4 mr-1" /> Record Inward</Button>}
      </div>

      {loading ? (
        <div className="space-y-2">{[1,2,3,4].map(i => <Skeleton key={i} className="h-14 bg-muted" />)}</div>
      ) : transactions.length === 0 ? (
        <Card><CardContent className="py-10 text-center text-muted-foreground">No inward transactions yet.</CardContent></Card>
      ) : (
          ...
          <table className="w-full min-w-[650px]">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground whitespace-nowrap">Supplier</th>
            ...
            <tbody>
              {transactions.map(tx => (
                <tr key={tx.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm font-semibold text-right text-success whitespace-nowrap">+{tx.quantity} {(tx.item as Item)?.unit}</td>
                  <td className="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap">{tx.supplier_vendor || '—'}</td>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg">
          <DialogHeader><DialogTitle>Record Stock Inward</DialogTitle></DialogHeader>
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-sm font-normal">Supplier / Vendor</Label>
                <Input placeholder="Supplier name" value={form.supplier_vendor} onChange={e => setForm(f => ({ ...f, supplier_vendor: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-sm font-normal">Reference / PO No.</Label>
